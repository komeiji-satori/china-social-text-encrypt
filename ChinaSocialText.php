...
class ChinaSocialText {
	public static function Encrypt($text = "") {
		...
		foreach ($seg_list as $key => $value) {
			if ($key % 2 == 0) {
				if (self::is_word($value) == 1) {
					$result_str .= $value;
				} else {
					$result_str .= self::EnglishTranslate($value);
				}
			} else {
	...
	private static function EnglishTranslate($chinese) {
		$headers = array(
	...
	private static function is_word($str = '') {
		if (trim($str) == '') {
			return '';
		}
